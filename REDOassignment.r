...
#Look at range of longitude values
summary(nat$Longitude)
nat[which(nat$Longitude >= 0), ]

#Three points appear to have the lat long numbers switched
#These can either be removed or swapped
swap <- nat[which(nat$Longitude >= 0), ]

nat$Latitude[which(nat$Longitude >= 0)] <- swap$Longitude
nat$Longitude[which(nat$Longitude >= 0)] <- swap$Latitude
